...
```{r}
mapview::mapview(b_list[[1]]) + mapview::mapview(b_list[[2]]) + mapview::mapview(b_list[[3]])
```
...
# cpsf$name[1]
# 
# oe_get(cpsf$name[1], boundary = sbox, download_directory = subDir, stringsAsFactors = FALSE, download_only = TRUE, skip_vectortranslate = TRUE, boundary_type = "clipsrc")
# 
# oe_get(cpsf$name[1], boundary = sbox, stringsAsFactors = FALSE, download_only = TRUE, boundary_type = "clipsrc")
# 
# lgeof <- st_read("/private/var/folders/nw/3_7tcnfd3sd0mzpcjkxljc5h0000gq/T/RtmptwZArl/geofabrik_London.gpkg")
...
#   st_drop_geometry() %>%
#   memdb_frame() %>%
#   filter(highway %in% c("NA")) %>%
#   show_query()
# 
# # query
# q <- "SELECT * FROM 'lines' WHERE (`highway` IN ('NA'))"
...
my_vectortranslate = c(
  ...
  "-where", "highway IN ('living_street','path','pedestrian','primary','primary_link','residential','secondary','secondary_link','service','tertiary','tertiary_link','trunk','trunk_link')"
)

# function to get osm data
for (o in 1:nrow(cpsf)) {
  print(b_list[o])
}
...
for (o in 1:3) {
  ...
  oe_get(cpsf[o,], boundary = b_list[[o]], vectortranslate_options = my_vectortranslate, stringsAsFactors = FALSE, download_only = TRUE, boundary_type = "clipsrc", max_file_size = Inf) -> ulist[o]
...
```{r}
# get data from point clipped to boundary & custom query
hosm <- oe_get(cpsf[3,], boundary = b_list[[3]], vectortranslate_options = my_vectortranslate, stringsAsFactors = FALSE, download_only = TRUE, boundary_type = "clipsrc")
...
st_layers("osm-data/test_its-example.osm.pbf", do_count = TRUE)

oe_get_keys("osm-data/test_its-example.osm.pbf", layer = "lines")
```
...
# clip a sample area
d <- 3000
# create point # http://bboxfinder.com/#51.521781,-0.126142,51.521781,-0.126142
point <- tibble(geometry = "POINT (530101.0040 181975.6229)") %>%
...
# boundary
sbox %>%
  st_set_crs(27700) -> sbox
# place
london = sf::st_sfc(sf::st_point(c(530101.0040, 181975.6229)), crs = 27700)
```

Download lines  
ref: https://docs.ropensci.org/osmextract/reference/oe_get.html  
```{r}
lon_lines <- oe_get(place = london, boundary = sbox, stringsAsFactors = FALSE, quiet = TRUE) # lines are downloaded by default # test: boundary_type = "clipsrc"
```

```{r}
q <- "SELECT * FROM 'multipolygons' WHERE building IS NOT NULL"
lon_building <- oe_get(place = london, boundary = sbox, query = q, stringsAsFactors = FALSE, quiet = TRUE)

ql <- "SELECT * FROM 'lines' WHERE highway IS NOT NULL"
lql <- oe_get(place = cpsf[2,], boundary = sbox, query = ql, stringsAsFactors = FALSE, quiet = TRUE)
```
...
```{r}
ffoot <- c('footway', 'sidewalk')
foot <- lon_lines[lon_lines$highway %in% ffoot,]
plot(foot["highway"])
```
...
plotpal <- function(col, border = "light gray", ...){
  n <- length(col)
  plot(0, 0, type="n", xlim = c(0, 1), ylim = c(0, 1),
       axes = FALSE, xlab = "", ylab = "", ..., asp = 1/8, mar = c(0,0))
  rect(0:(n-1)/n, 0, 1:n/n, 1, col = col, border = NA, lwd=0.01) # border = border (for light grey border)
}
```

```{r}
two <- c('#1f306e', '#553772', '#8f3b76', '#c7417b', '#f5487f')
mycols <-  rev(c("#3B9AB2", "#78B7C5", "#EBCC2A", "#E1AF00", "#F21A00"))
colorRampPalette(two)(180)
```

```{r}
plotpal(colorRampPalette(two)(10))
plotpal(colorRampPalette(c("#ffbe0b","#fb5607","#ff006e","#8338ec","#3a86ff"))(180))
```

```{r}
c(colorRampPalette(mycols)(180),colorRampPalette(mycols)(180))
```
...
```{r}
plotpal(carto_pal(12, "Bold"))
```


```{r}
plotpal(c(colorRampPalette(mycols)(180),colorRampPalette(mycols)(180)))
```
...
lonh %>%
  mutate(hcol = case_when(heading > 90 & heading < 180 ~ heading - 90,
                          ...
                          heading > 270 & heading < 360 ~ heading - 270,
                          TRUE ~ heading)) %>%
  mutate(cut = cut_number(hcol, n = 90)) %>%
  st_transform(27700) %>%
  st_crop(.,sbox) %>%
  #sample_n(5000) %>%
  ggplot() +
  geom_sf(aes(col = cut), size = 0.4, show.legend = FALSE) +
  scale_colour_manual(values = c(colorRampPalette(carto_pal(12, "Bold"))(90))) +
  theme_void()
```
