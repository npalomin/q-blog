...
for (p in 1:nrow(df)) {
  ...
  df %>%
    ...
    st_bbox() %>%
    st_as_sfc() %>%
    st_as_sf() -> b_list[cpsf$name[p]]
}

lapply(b_list, st_as_sf) -> b_list1

# Convert a list of sf objects into one sf
do.call(rbind, b_list1) %>%
  mutate(name = cpsf$name) %>%
  rename(geometry = x)-> bboxes

st_write(bboxes, "data/bboxes.gpkg")
```
